...
import { lireUtilisateurStocke } from '@/shared/session/useAuthSession'
...
<template>
  <main class="ecs-page-bg text-[#0f172a]">
    <div class="min-h-screen">
      ...
      <aside
        class="fixed inset-y-0 left-0 z-50 w-[274px] border-r border-white/60 bg-white/90 px-4 pb-5 pt-4 shadow-[0_22px_55px_rgba(15,23,42,0.12)] backdrop-blur-xl transition-transform lg:translate-x-0"
        :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
      >
        <div class="mb-4 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <img :src="logoEasyClubSportMark" alt="EasyClubSport" class="h-9 w-9 rounded-2xl border border-[#d9e4ff] bg-white p-1.5 shadow-[0_10px_20px_rgba(15,23,42,0.08)]" />
            <div>
        ...
        <nav class="mt-4 space-y-1.5">
          <RouterLink
            ...
            :key="link.id"
            :to="link.to"
            class="ecs-sidebar-link"
            :class="
              activeSection === link.id
                ? 'ecs-sidebar-link-active'
                : ''
            "
            @click="sidebarOpen = false"
          >
        ...
        <div class="mt-6 rounded-[28px] border border-white/40 p-4 text-white shadow-[0_12px_24px_rgba(15,23,42,0.35)]" style="background: linear-gradient(145deg, #111827 0%, #172554 44%, #1d4ed8 78%, #14b8a6 100%);">
          ...
          <RouterLink
            ...
            class="mt-4 block rounded-full bg-white/95 px-3 py-2 text-center text-[11px] font-bold text-[#0f172a]"
            @click="sidebarOpen = false"
          >
            OUVRIR DASHBOARD
          </RouterLink>
      ...
      <section class="px-4 pb-6 pt-4 sm:px-6 lg:ml-[274px] lg:px-7">
        <header class="mb-5 rounded-[30px] border border-white/70 bg-white/85 px-4 py-3 shadow-[0_18px_32px_rgba(15,23,42,0.08)] backdrop-blur-xl sm:px-5">
            ...
            <div class="flex items-center gap-3">
              <label class="relative hidden sm:block">
                ...
                <input
                  ...
                  class="h-10 w-[260px] rounded-full border border-[#dbe2ef] bg-white px-3 py-2 pl-10 text-sm text-[#1f2a44] outline-none transition placeholder:text-[#94a3b8] focus:border-[#2563eb] focus:ring-2 focus:ring-[#2563eb]/15"
                />
              </label>

              <button class="relative rounded-full border border-[#dbe2ef] bg-white px-3 py-2 text-xs font-semibold text-[#475569]" type="button" aria-label="Notifications">
                Alertes
                <span class="absolute -right-1 -top-1 h-2.5 w-2.5 rounded-full bg-[#ef4444] ring-2 ring-white"></span>
              </button>

              <div class="group relative">
                <button class="flex items-center gap-2 rounded-full border border-[#dbe2ef] bg-white px-2 py-1.5" type="button" aria-label="Profile menu">
                  <AppAvatar
                    :src="profilConnecte.image"
                    :alt="profilConnecte.nom"
                    :initials="profilConnecte.initiales"
                    size-class="h-8 w-8"
                ...
                <div class="invisible absolute right-0 top-11 z-40 w-[255px] rounded-[28px] border border-[#dbe2ef] bg-white p-4 opacity-0 shadow-[0_22px_40px_rgba(15,23,42,0.18)] transition duration-200 group-hover:visible group-hover:opacity-100">
                  ...
                  <RouterLink to="/president/profil" class="mt-3 block rounded-full px-3 py-2 text-sm font-semibold text-[#475569] transition hover:bg-[#eff6ff] hover:text-[#1d4ed8]">
                    Voir profil
                  </RouterLink>
