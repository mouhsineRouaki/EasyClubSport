  ...
  <section class="rounded-[22px] bg-white p-4">
    <div class="flex items-center justify-between gap-3">
      ...
      <button
        v-if="coach"
        type="button"
        class="rounded-full border border-[#fecaca] px-3 py-1 text-[10px] font-black text-[#ef4444] transition hover:bg-[#fef2f2] disabled:opacity-50"
        :disabled="chargement"
        @click="emit('retirer')"
      >
        Retirer
    ...
    <div v-else class="mt-4">
      ...
      <button
        type="button"
        class="mt-3 w-full inline-flex items-center justify-center gap-2 rounded-full bg-[#111827] px-4 py-2.5 text-xs font-black text-white transition hover:bg-[#2446d8] disabled:opacity-50"
        :disabled="chargement"
        @click="emit('assigner')"
      >
