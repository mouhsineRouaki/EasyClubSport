...
const imageEvenement = (ev = {}) => {
  return ev.image_url || ev.photo_url || ev.image || ev.equipe?.logo_url || ev.adversaire_equipe?.logo_url || props.equipe?.club?.logo_url || blueBackground
}

const backgroundEvenement = (ev = {}) => {
  return `linear-gradient(180deg, rgba(7,16,58,0.14), rgba(7,16,58,0.88)), url(${imageEvenement(ev)})`
}
...
const detailsEvenement = computed(() => {
  if (!evenementSelectionne.value) return []

  return [
    { label: 'Date debut', value: formatDateHeure(evenementSelectionne.value.date_debut) },
    { label: 'Date fin', value: evenementSelectionne.value.date_fin ? formatDateHeure(evenementSelectionne.value.date_fin) : 'Non definie' },
    { label: 'Lieu', value: evenementSelectionne.value.lieu || '-' },
    { label: 'Type', value: evenementSelectionne.value.type || '-' },
  ]
})
...
const detailsEquipe = computed(() => {
  ...
  return [
    {
      label: 'Club',
      value: evenementSelectionne.value.equipe?.club?.nom || props.equipe?.club?.nom || '-',
      helper: evenementSelectionne.value.equipe?.club?.ville || props.equipe?.club?.ville || 'Ville non definie',
    },
    {
      label: 'Equipe',
      value: evenementSelectionne.value.equipe?.nom || props.equipe?.nom || '-',
      helper: evenementSelectionne.value.equipe?.categorie || props.equipe?.categorie || 'Categorie non definie',
    },
  ]
})
  ...
  <section class="mt-6">
    ...
    <div v-if="chargement" class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <div
        v-for="n in 6"
        :key="n"
        class="h-[270px] animate-pulse rounded-[30px] bg-[linear-gradient(120deg,#f8fbff,#eef3ff,#f8fbff)]"
      ></div>
    </div>
    ...
    <template v-else-if="mode === 'detail' && evenementSelectionne">
      <div class="mt-6 rounded-[30px] border border-[#e6edf8] bg-white p-4 sm:p-5">
        <div class="mb-4 flex items-center justify-between gap-3">
          ...
          <span class="rounded-full px-4 py-2 text-xs font-black capitalize" :class="badgeStatut(evenementSelectionne.statut).cls">
        ...
        <div class="overflow-hidden rounded-[28px] border border-[#e6edf8]">
          <div class="min-h-[260px] bg-cover bg-center p-5 text-white sm:p-6" :style="{ backgroundImage: backgroundEvenement(evenementSelectionne) }">
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span
                v-if="evenementSelectionne.convocation || evenementSelectionne.ma_convocation"
              ...
              >
                {{ badgeConvocation((evenementSelectionne.convocation || evenementSelectionne.ma_convocation).statut).label }}
              </span>
            </div>
            ...
            >
              <div class="min-w-0 text-center">
                <img
                  v-if="logoEquipe(evenementSelectionne.equipe || equipe)"
              ...
              <div class="min-w-0 text-center">
                <img
                  v-if="logoEquipe(evenementSelectionne.adversaire_equipe)"
          ...
          <div class="bg-white p-5">
            <AppDetailsTable :items="detailsEvenement" :columns="4" />
          </div>
          ...
          <div class="border-t border-[#eef2f8] bg-white p-5">
            <article class="rounded-[26px] border border-[#e8edf6] bg-white p-5">
              <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <span class="rounded-full px-3 py-1.5 text-[10px] font-black" :class="badgeDisponibilite(evenementSelectionne.disponibilite?.reponse).cls">
                  {{ badgeDisponibilite(evenementSelectionne.disponibilite?.reponse).label }}
                </span>
              </div>

              <textarea
                v-model="commentaireDisponibilite"
                rows="3"
                ...
                placeholder="Ajouter un commentaire pour votre coach..."
              ></textarea>

              <div class="mt-4 flex flex-wrap gap-2">
                <AppButton type="button" variant="neutral" size="sm" :disabled="envoiDisponibilite" @click="repondreDisponibilite('present')">
                ...
                <AppButton
                  ...
                  variant="ghost"
                  size="sm"
                  class="!text-[#f59e0b] !bg-[#fff7ed] !border-[#fde7c2] hover:!bg-white"
                  :disabled="envoiDisponibilite"
                  @click="repondreDisponibilite('incertain')"
                >
                  Je suis incertain
                </AppButton>
                <AppButton type="button" variant="danger" size="sm" :disabled="envoiDisponibilite" @click="repondreDisponibilite('absent')">
                  Je suis absent
                </AppButton>
              </div>
            </article>
    ...
    <div v-else-if="evenementsFiltres.length" class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      >
        ...
        <div v-else class="mt-4 rounded-[22px] bg-[#f5f7fb] p-3">
          ...
          <p class="mt-1 truncate text-xs font-semibold text-[#64748b]">{{ ev.equipe?.nom || equipe?.nom || 'Equipe' }}</p>
        </div>

        <div class="mt-4 flex flex-wrap items-center justify-between gap-2">
          <div class="flex flex-wrap items-center gap-2">
            <span class="rounded-full px-3 py-1.5 text-[10px] font-black" :class="badgeDisponibilite(ev.disponibilite?.reponse).cls">
              {{ badgeDisponibilite(ev.disponibilite?.reponse).label }}
            </span>
            <span
              v-if="ev.convocation || ev.ma_convocation"
          ...
          <AppButton type="button" variant="neutral" size="sm" @click.stop="ouvrirDetail(ev)">
            Ouvrir
          </AppButton>
        </div>
